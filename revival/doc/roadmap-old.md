# AUIS Revival Roadmap (retired 2026-08-07)

**Superseded by `roadmap.md`.** This file is the pre-2026-08-07 roadmap,
kept verbatim for its blow-by-blow investigation history and provenance —
it is not maintained going forward and parts of it are stale (e.g. the
strict-prototypes follow-on it describes as "sized but not scheduled" was
completed and closed out the same day this file was retired). For current
status, use `roadmap.md`. For the full dated history, `porting-changelog.md`
and `porting-assessment.md` remain current.
