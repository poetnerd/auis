...
client-server split.  The *message server* is a process which runs on 
...
*client* program, such as the cui or messages programs.  Clients never 
...
The most basic entity in the message system is, naturally, a *message*. 
...
that standard, each message contains a set of *headers* (which may be 
viewed as attribute/value pairs, and can be retrieved as such by the client 
from the message server), and a *body*, which is the text of the 
message.


Messages are grouped together in *message directories*.  A message 
...
*snapshot*.  Snapshots are the structure most commonly exchanged 
...
The *caption* is a one line summary of the message, to be shown, for 
...
The *id* is a unique identifier that is associated with the message. 
...
The *date* is an ordering field, by which messages in a directory are 
...
The *attributes* of a message is a set of bits encoding certain facts 
...
The *chain* of a message is a network-byte-order integer that can be 
...
bcopy(AMS_CHAIN(SBuf), &refint, sizeof(unsigned long));
...
## Message Server Routines Available to Clients
...
*A note about convention*:  For each 
...
*A note about return codes*:  All message 
...
**AMS_ERRNO**: The actual error code.  This will be either a value 
...
**AMS_ERRCAUSE**:  The local context in which the error occurred, typically 
...
**AMS_ERRVIA**:  The larger context in which the error occurred, typically 
...
**AMS_RPCERRNO**:  The error code for errors in SNAP remote procedure 
...
## Calls that relate to information about messages


`MS_HeadersSince(FullDirName, datefield, ReturnBuf, 
...
`MS_GetPartialBody(DirName, id, Buf, BufLim, offset, 
remaining, ct)
...
specifically, it will get the nth snapshot in the named 
...
`MS_GetHeaderContents(dirname, id, HeaderName, 
...
**ASS_REPLACE_ALL**:  The snapshot is simply replaced by the 
new snapshot.


**ASS_REPLACE_ATTRIBUTES**:  The attributes are replaced by the attributes 
in the new snapshot, but everything else is left unchanged.


**ASS_REPLACE_ATT_CAPT**:  The attributes AND caption fields are replaced 
by those in the new snapshot, and everything else is left unchanged.


**ASS_OR_ATTRIBUTES**:  The attributes in the two snapshots are OR'ed 
together, with everything else left unchanged.  (This is used, for example, to 
mark a message as deleted.)


**ASS_AND_ATTRIBUTES**:  The attributes in the two snapshots are AND'ed 
together, with everything else left unchanged.  (This is used, for example, to 
...
**MS_CLONE_COPY**: The message is simply copied to the new 
directory.


**MS_CLONE_COPYDEL**: The message is copied, and marked for deletion in the 
old directory.


**MS_CLONE_APPEND**: The message is copied to the new directory and the 
date field is changed to position the message at the end of that directory 
(the date is set to the current time or greater).


**MS_CLONE_APPENDDEL**: The message is appended as in MS_CLONE_APPEND, and 
marked for deletion in the old directory.


**MS_CLONE_SYMLINK**:  (NOT YET IMPLEMENTED) The message is created in the 
...
`long    MS_EditMessage (dirname, id, NewBodyFile, 
...
**AMS_PRINT_FIXED** -- Print the body in a fixed-width font

**AMS_PRINT_ROT13** -- Print the body using the netnews Rot13 simple 
decryption scheme.
...
This routine deletes all messages from a message directory *and all its 
...
## Calls that relate to information about message directories


`MS_CreateNewMessageDirectory(DirName, Overwrite, 
BodyDirName)
...
`MS_ReconstructDirectory(DirName, NumGood, NumBad, 
TrustTimeStamp)
...
`MS_ScavengeDirectory((DirName, Recurse, numgood, numbad, 
quiet, Purge)
...
**AMS_DIRPROT_READ**: The user has read-only access, and the 
directory is neither a local, external, nor official bboard.


**AMS_DIRPROT_LOCALBB**: The user has read-only access, and the directory 
...
**AMS_DIRPROT_OFFBB**: The user has read-only access, and the directory is 
in the $OFFICIAL domain, as defined previously for $LOCAL.


**AMS_DIRPROT_EXTBB**: The user has read-only access, and the directory is 
in the $EXTERNAL domain, as defined previously for $LOCAL.


**AMS_DIRPROT_MODIFY**: The user has read and write access.


**AMS_DIRPROT_MBOX**: The user has read and write access, and may also 
process incoming mail from the associated Mailbox.


**AMS_DIRPROT_FULLMAIL**: The user has read and write access, and may also 
process incoming mail from the associated Mailbox, and it is one of his 
private mail directories.


**AMS_DIRPROT_AWFUL**:  The user has no access, or the combination of 
...
`MS_InstallWelcomeMessage(ParentName, InitDir, InitFile, 
ShortName)
...
of the subdirectory within the parent directory (*not* the full path 
name, simply the last element of the name of the new directory).


`MS_MatchFolderName(pat, fname)
...
## Calls that relate to mail delivery
...
**AMS_REPLY_FRESH**: The DirName and id are ignored, and a 
...
**AMS_REPLY_SENDER**:  The mail is generated as a reply to the sender of 
the original message.


**AMS_REPLY_WIDE**: The mail is generated as a reply to the recipients of 
...
**AMS_REPLY_WIDER**:  The mail is generated as a reply to both the sender 
and recipients of the original message.


**AMS_REPLY_FORWARD**:  The mail is generated as an attempt to forward the 
...
**AMS_REPLY_FORWARD_FMT**:  Same as AMS_REPLY_FORWARD, except that BE2 
formatting is preserved; this should only be used by interfaces that 
understand the BE2 datastream.


**AMS_REPLY_REDRAFT**:  The mail is considered to be a draft message which 
is now to be considered again for sending.  Several headers and the body are 
filled in.


`MS_ValidateAndReplaceChunk(FileName, inaddr, outaddr, 
...
`MS_SubmitMessage(FileName, DeliveryOptions, ErrorMessage, 
...
**AMS_SEND_BLINDYES** -- Keep a blind copy.

**AMS_SEND_BLINDNO** -- Do not keep a blind copy.

**AMS_SEND_ISRESEND** -- Write "ReSent-From" instead of "From", etc.

**AMS_SEND_ILLEGAL** -- Don't complain about non-Ascii bytes or long lines

**AMS_SEND_UNFORMATTED** -- Strip out any Andrew Toolkit (BE2) formatting 
before sending
...
## Calls that relate to remote file access
...
In general, client programs should *not ever* call MS_Disambiguate file 
...
`MS_GetPartialFile(FileName, Buf, BufLim, offset, 
remaining, ct)
...
`MS_StorePartialFile(FileName, startpos, len, mode, 
...
## Calls that relate to personal subscription/profile information
...
`MS_NameChangedMapFile(MapFile, MailOnly, ListAll, 
...
`MS_RebuildMasterUpdateFiles(NumFastGood, NumSlowGood, 
...
`MS_RebuildOneMasterUpdateFile(PathElt, NumFastGood, 
...
## Calls That Relate To User-Defined Attributes
...
## Calls primarily for debugging purposes
...
`MS_GetConfigurationParameters (MailDomain, len, 
...
`MS_ProcessNewMessages(SourceDir, NumGood, NumBad, 
...
available from the author; it should be used *instead* of the convert 
...
`MS_PrefetchMessage(DirName, id, GetNext)
...
`MS_GetNewMessageCount(FullDirName, numnew, numtotal, 
...
`MS_ParseDate(indate, year, month, day, hour, min, sec, 
wday, gtm)
...
`MS_HandlePreference(prog, pref, InVal, OutVal, OutLim, 
