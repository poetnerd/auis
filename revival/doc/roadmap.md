...
| Core embedded insets — `eq`, `table`, `fad`, `fnote`, `bp`, `srctext`, `figure`, `value`/slider/button, `pushbutton`/`link`, `lset`, `org`, `apt`/`cel`/`arbiter` | Fully working | `figure` has one open cosmetic bug: italic-text sizing is non-monotonic (10pt renders bigger than 12pt). `eq` has one open bug: deleting a multi-line equation's lines only reduces each to a single "zilch" placeholder rather than merging down to one line — a long-standing bug, not new to this port (see `porting-assessment.md` §23). `arbiter`: a 35-year `short`-overflow that made a complex arb demo (an embedded piano keyboard) render as nothing was fixed 2026-08-21, but exposed a second, deeper, still-open sizing bug — the same demo's height goes pathological on any relayout after first paint (see `revival.md` → "Open issues") |
