...
- Menu-bar posting sometimes takes a fixed ~510ms quantum to draw (perceived multi-second delays are two or more of these back-to-back, not one longer stall): confirmed via live process sampling to be a genuine X-server round-trip stall inside XQuartz itself, not AUIS code — six candidate causes tested and eliminated, including reproducing `eza`'s exact window/font/save-under behavior in a standalone C program against the same live server, which never showed the delay. Leading theory as of 2026-08-09 (client X-resource footprint) was undercut by the ~510ms value's quantization (a timer/coalescing signature, not a scaling-cost one) once flagged by a second-opinion review. As of 2026-08-11: App Nap opt-out (`NSAppSleepDisabled`) tested and ruled out specifically; live-sampling XQuartz's own `X11.bin` process during a real stall caught it blocked in a concrete named chain — `ProcClearToBackground` (servicing `DrawMenuItems`'s `XClearWindow`) → `RootlessStartDrawing` → `xp_lock_window` → `SLSConnectionSynchronizeSLSCATransaction` → `_SLSTransactionWaitSource` — i.e. waiting on macOS's own SkyLight/Core Animation window compositor to synchronize a transaction before the server can lock the window's backing store to draw. Leading theory is now this specific mechanism (WindowServer commit-cadence throttling for a non-frontmost app), not generic idle-throttling. See `porting-assessment.md` → "Intermittent ~500ms menu-bar posting delay" for the full inventory, this finding, and remaining follow-up tests, none needing a second X server. Diagnostic tracing left in the tree, off by default (`MenuDebugTrace` profile switch; `MENUDBGTRACE` env var for the popup-menu path).
