...
void eintview_CancelCommand(struct eintview *self)
{
...
void eintview_ReturnCommand(struct eintview *self)
{
    struct enterint *txt = Text(self);
...
void eintview_ClearCommand(struct eintview *self)
{
    struct enterint *txt = Text(self);
    enterint_Clear(txt);
    enterint_NotifyObservers(txt,0);
...
void eintview__ReceiveInputFocus(struct eintview *self)
{
...
void eintview__LoseInputFocus(struct eintview *self)
{
    if(self->valueview)
...
boolean eintview__InitializeClass(struct classheader *classID, struct eintview *self)
{
