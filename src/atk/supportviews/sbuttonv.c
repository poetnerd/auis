...

void sbuttonv__HighlightButton(struct classheader *classID, struct view *self, struct sbutton_info *si, struct rectangle *r)
{
    struct rectangle Rect2;
    struct fontdesc *my_fontdesc;
    int tx, ty;
...

void sbuttonv__UnHighlightButton(struct classheader *classID, struct view *self, struct sbutton_info *si, struct rectangle *r)
{
    struct rectangle Rect2;
    int tx, ty;
    int bdepth, r2_bot, r_bot;
...

void sbuttonv__ObservedChanged(struct sbuttonv *self, struct observable *b, long v)
{
	...
	if(sbutton_GetTrigger(b2, bchange)) {
	    observable_DefineTrigger(self, sbutton_GetTrigger(b2, bchange));
	    return;
	}
    }
...

enum view_DSattributes sbuttonv__DesiredSize(struct sbuttonv *self, long width, long height, enum view_DSpass pass, long *desired_width, long *desired_height)
{
    /* 
...
void sbuttonv__SetDataObject(struct sbuttonv *self, struct dataobject *b)
{
    if(sbutton_GetMaxCount((struct sbutton *)b)==0) sbutton_EnsureSize((struct sbutton *)b, 0);
    sbutton_Enumerate((struct sbutton *)b, definetriggers, self);
...
struct sbuttonv * sbuttonv__CreateFilledSButtonv(struct classheader *classID, char *defview, struct sbutton_prefs *prefs, struct sbutton_list *blist)
{
...
void sbuttonv__Print(struct sbuttonv *self, FILE *file, char *processor, char *format, boolean topLevel)
{
	int count;
