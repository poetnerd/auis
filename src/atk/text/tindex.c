...
void tindex__PrintIndex(struct classheader *ClassID, struct view *self)
{
    if(self == NULL) return;
...
void tindex__PreviewIndex(struct classheader *ClassID, struct view *self)
{
    if(self == NULL) return;
    message_DisplayString(NULL,0,"Preparing Index - please wait");
    environ_Put("IndexOnly",NULL);
...
void tindex_ReadIndexFile(struct view *v)
{
    /* Prompt for an index term and call index_IndexTerm */
    FILE *f,*fopen();
...
void tindex_WriteIndexFile(struct view *v)
{
    /* Prompt for an index term and call index_IndexTerm */
    FILE *f,*fopen();
...
void tindex_MakeIndexPlain(struct view *v)
{
    struct textview *self;
    if((self = getrealview(v)) == NULL) return ;
    tindex_FudgeFonts(Text(self),"index",fontdesc_Plain);
}
void tindex_MakeIndexItalic(struct view *v)
{
    struct textview *self;
    if((self = getrealview(v)) == NULL) return ;
    tindex_FudgeFonts(Text(self),"index",fontdesc_Italic);
}
void tindex_HideInvIndex(struct view *v)
{
    struct textview *self;
    if((self = getrealview(v)) == NULL) return ;
    tindex_FudgeFonts(Text(self),"indexi", tindex_HIDDEN);
}
void tindex_ExposeInvIndex(struct view *v)
{
    struct textview *self;
    if((self = getrealview(v)) == NULL) return ;
