...
void figogrp__FinalizeObject(struct classheader *classID, struct figogrp *self)
{
    return;
}
...
enum figobj_Status figogrp__Build(struct figogrp *self, struct figview *v, enum view_MouseAction action, long x, long y, long clicks)
{
...
enum figobj_HandleType figogrp__GetHandleType(struct figogrp *self, int num)
{
