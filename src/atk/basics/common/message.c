...
int message__AskForStringCompleted(struct classheader *classID, struct view *view, int priority, char *prompt, char *defaultString, char *buffer, int bufferSize, struct keymap *keystate, procedure completionProc, procedure helpProc, void *functionData, int flags)
{
    char *str = messitem_Replace(prompt);
    struct msghandler *handler;
