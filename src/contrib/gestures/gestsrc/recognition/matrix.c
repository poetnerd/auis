...
#ifndef NORCSID
#define NORCSID
static char rcsid[]="$Header: /afs/cs.cmu.edu/project/atk-dist/auis-6.3/contrib/gestures/gestsrc/recognition/RCS/matrix.c,v 1.7 1993/12/07 17:14:18 rr2b Exp $";
#endif
...
int r;
{
	register union array_header *a;
	register Vector v;

	a = (union array_header *)
	    allocate(sizeof(union array_header) + r * sizeof(double), char);
	a->ah.ndims = 1;
	a->ah.nrows = r;
	a->ah.ncols = 1;
	v = (Vector) (a + 1);
...
int r, c;
{
	register union array_header *a = (union array_header *)
	   allocate(sizeof(union array_header) + r * sizeof(double *), char);
	register int i;
	register Matrix m;
...
register Vector v1, v2;
{
	...
	if(n != NROWS(v2))
		error("InnerProduct %d x %d ", n, NROWS(v2));
...
register Matrix m;
{
	...
	if(n != NROWS(m) || n != NCOLS(m))
		...
		error("QuadraticForm: bad matrix size (%dx%d not %dx%d)",
			NROWS(m), NCOLS(m), n, n);
...
Matrix ym, rm;
{
	...
	if(NROWS(ym) != NCOLS(ym))
		error("InvertMatrix: not square");

	n = NROWS(ym);

	if(n != NROWS(rm) || n != NCOLS(rm))
		error("InvertMatrix: result wrong size");
	...
	if (n >= PERMBUFSIZE)
		error("InvertMatrix: PERMBUFSIZE");
...
FILE *f;
{
	...
	if(fscanf(f, "%1s %d", check, &nrows) != 2)
		error("InputVector fscanf 1");
	if(check[0] != 'V')
		error("InputVector check");
	v = NewVector(nrows);
	for(i = 0; i < nrows; i++)
		if(fscanf(f, "%lf", &v[i]) != 1)
			error("InputVector fscanf 2");
	return v;
}
...
FILE *f;
{
	...
	if(fscanf(f, "%1s %d %d", check, &nrows, &ncols) != 3)
		error("InputMatrix fscanf 1");
	if(check[0] != 'M')
		error("InputMatrix check");
	m = NewMatrix(nrows, ncols);
	for(i = 0; i < nrows; i++)
		for(j = 0; j < ncols; j++)
			if(fscanf(f, "%lf", &m[i][j]) != 1)
				error("InputMatrix fscanf 2");
	return m;
}
