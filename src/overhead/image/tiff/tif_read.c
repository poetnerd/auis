...
static int TIFFReadRawStrip1(TIFF *tif, u_int strip, u_char *buf, u_int size, char module[])
{
	TIFFDirectory *td = &tif->tif_dir;
...
static int TIFFReadRawTile1(TIFF *tif, u_int tile, u_char *buf, u_int size, char module[])
{
	TIFFDirectory *td = &tif->tif_dir;
 ...
 * raw data.
 */
int TIFFReadBufferSetup(TIFF *tif, char *bp, u_int size)
{
