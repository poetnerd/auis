...
#define OT_INTEGER 1
#define OT_SPECIAL 3
/* like OT_EXPLEVEL (an on/off toggle) but reads/writes a plain,
   app-agnostic profile key directly instead of a bit packed into
   messages' own "messages.BinaryOptions" bitmask -- so cui (and any
   other ATK program) can read the same key with a plain
   environ_GetProfileSwitch() call, with no messages-specific decoding.
   See ProfSwitchHit(). */
...
#define OT_PROFSWITCH_BLOCKBEACONS 2
...
static void ProfSwitchKey(int parm, char **app, char **pref)
{
	...
	case OT_PROFSWITCH_LOADREMOTEIMAGES:
	    *app = "ams"; *pref = "loadremoteimages";
	    break;
	case OT_PROFSWITCH_BLOCKBEACONS:
	    *app = "ams"; *pref = "blockbeaconimages";
	    break;
...
/* Default value for a given OT_PROFSWITCH row's OptParm, when the
   user has never set the underlying profile key -- kept next to
   ProfSwitchKey() since the options-dialog checkbox display below
   needs to agree with whatever default value(s) other code reading
   the same profile key uses (e.g. htmlatk.c's own
   environ_GetProfileSwitch("ams.blockbeaconimages", TRUE) check);
   disagreeing would show the checkbox as OFF while the feature it
   controls actually behaves as ON, until the user touches it once. */
static boolean ProfSwitchDefault(int parm)
{
    switch (parm) {
	case OT_PROFSWITCH_BLOCKBEACONS:
	    return TRUE;
...
int ProfSwitchHit(long self, struct value *val, int which, int hisrock)
{
    ...
    newval = !environ_GetProfileSwitch(key, ProfSwitchDefault(Options[which].OptParm));
...
*/
	...
	if (Options[i].OptType == OT_EXPLEVEL) {
	    ...
	    value_SetValue(v, environ_GetProfileSwitch(psKey, ProfSwitchDefault(Options[i].OptParm)) ? 1 : 0);
